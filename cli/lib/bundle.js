...
async function bundleHtml(cartFile, htmlFile, opts) {
  ...
  const htmlFileContent = Mustache.render(template, {
    html: {
      ...
      diskPrefix:
        opts.htmlDiskPrefix != null ? opts.htmlDiskPrefix : opts.title,
      wasmCartJson,
      wasm4Css,
      wasm4js,
      iconUrl,
...
async function bundle(cartFile, opts) {
  ...
  if (stat.size > limit) {
    console.warn(
      `Cart size of ${stat.size} bytes is above the limit of ${limit} bytes.`
    );
  }
  ...
  if (opts.windows) {
    await bundleExecutable(
      cartFile,
      path.resolve(__dirname, "../assets/natives/wasm4-windows.exe"),
      opts.windows,
      opts
    );
  }
  if (opts.mac) {
    await bundleExecutable(
      cartFile,
      path.resolve(__dirname, "../assets/natives/wasm4-mac"),
      opts.mac,
      opts
    );
  }
  if (opts.linux) {
    await bundleExecutable(
      cartFile,
      path.resolve(__dirname, "../assets/natives/wasm4-linux"),
      opts.linux,
      opts
    );
  }
